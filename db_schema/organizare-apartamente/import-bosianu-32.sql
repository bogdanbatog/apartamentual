...
select * from (
  select 1 as ord, 'grup' as sectiune, g.id::text as id,
   ...
   where g.nume ilike '%Constantin Bosianu%'
  union all
  ...
  select 3, 'membru activ', m.user_id::text,
   ...
   where m.grup_id in (select id from public.grupuri
                        where nume ilike '%Constantin Bosianu%')
     and m.status = 'activ'
  union all
  select 4, 'favorit (terenuri_likes_grupuri)', l.teren_id::text,
   ...
   where l.grup_id in (select id from public.grupuri where nume ilike '%Constantin Bosianu%')
  union all
  select 5, 'analiză deja existentă', a.id::text, a.titlu,
   ...
   where a.grup_id in (select id from public.grupuri where nume ilike '%Constantin Bosianu%')
  union all
  select 6, 'legat vechi (grup_terenuri)', gt.teren_id::text,
   ...
   where gt.grup_id in (select id from public.grupuri where nume ilike '%Constantin Bosianu%')
...
select
  (select id from public.grupuri where nume ilike '%Constantin Bosianu%'),
...
update public.analiza_varianta va
   set pdf_path = (select id::text from public.grupuri where nume ilike '%Constantin Bosianu%') || '/' || 'bosianu-32-fisa.pdf',
       pdf_nume = 'bosianu-32-fisa.pdf',
       kml_path = (select id::text from public.grupuri where nume ilike '%Constantin Bosianu%') || '/' || 'bosianu-32-volum.kml',
       kml_nume = 'bosianu-32-volum.kml'
  from public.analiza_teren a
 where a.id = va.analiza_id
   and a.titlu = 'Analiză preliminară Constantin Bosianu 32'
   and a.grup_id = (select id from public.grupuri where nume ilike '%Constantin Bosianu%')
...
--  where titlu = 'Analiză preliminară Constantin Bosianu 32'
--    and grup_id = (select id from public.grupuri where nume ilike '%Constantin Bosianu%');
--
