-- ═══════════════════════════════════════════════════════════════════════════
-- `lot_terenuri_noi` — VERSIUNEA 2: întoarce și terenurile, nu doar cifrele
-- ═══════════════════════════════════════════════════════════════════════════
--
-- ⚠️ ÎNLOCUIEȘTE BLOCUL 5 din `2-baza.sql`. Acela NU se mai rulează.
--    Fișierul de acolo rămâne pentru istoric și pentru celelalte blocuri
--    (bifa, jurnalul, drepturile), care sunt neschimbate și deja rulate.
--
-- DE UNDE A PORNIT. Textul emailului, decis pe 13 august, îi arată omului
-- terenurile concrete apărute în zonele lui — fiecare cu titlu, suprafață,
-- preț și link direct către pagina lui. Versiunea 1 a funcției întorcea doar
-- numere („7 terenuri noi în Carol"), iar pagina de terenuri **nu citește
-- parametri din URL** (verificat în `js/terenuri.js`), deci nici măcar un
...
-- nume și același înțeles — deci nimic din ce s-a dovedit cu `2b` nu se pierde.
--
-- ⚠️ NU pune BEGIN / ROLLBACK în acest script.
-- ⚠️ RULEAZĂ BLOCURILE PE RÂND. BLOC 0 se citește înainte de orice altceva.
-- ═══════════════════════════════════════════════════════════════════════════


-- ───────────────────────────────────────────────────────────────────────────
-- BLOC 0 — INVENTAR: cum se numesc, de fapt, coloanele lui `terenuri`
-- ───────────────────────────────────────────────────────────────────────────
-- ⚠️ NU LE POT AFLA DIN COD: `js/terenuri.js:216` face `select('*')`, deci
--    numele nu apar nicăieri scrise. Cele folosite la desenarea cardurilor
--    (`terenuri.js:333-401`) sunt `titlu`, `suprafata`, `pret_total`,
--    `image_url`, `image_urls` — dar aia e ce citește JavaScript-ul din
--    obiect, nu o dovadă că așa se numesc în bază.
--
-- Dacă un nume diferă, funcția din BLOC 1 crapă cu `42703: column does not
-- exist`. Deci: rulează asta ÎNTÂI și trimite-mi rezultatul.

select column_name, data_type, is_nullable
from information_schema.columns
where table_schema = 'public'
  and table_name   = 'terenuri'
order by ordinal_position;

-- ✅ RULAT 13 august (CSV 97). Numele se potrivesc: `titlu` (varchar),
--    `suprafata` (integer), `pret_total` (numeric), `image_url` (varchar),
--    `image_urls` (ARRAY — tablou Postgres, nu jsonb, deci se ia `[1]`).
--
-- ⚠️ TREI CAPCANE GĂSITE ÎN INVENTAR, de ținut minte:
--
--   1. `terenuri` are DOUĂ coloane de dată: `created_at` ȘI `data_adaugat`,
--      amândouă `not null`. Fereastra noastră merge pe `created_at`, la fel ca
--      interogarea validată în august și ca sortarea „Cele mai noi" din
--      `terenuri.js:218`. Dacă vreodată cineva „corectează" filtrul pe
--      `data_adaugat` fiindcă sună mai potrivit, fereastra se poate muta tăcut.
--
--   2. `terenuri` are ȘI `zona` (varchar), ȘI `cartier` (text). Potrivirea cu
--      tabela `zones` se face pe `cartier` — `zona` pare rămasă din schema
--      veche. Nu o atinge și nu o folosi la potrivire.
--
--   3. `pret_pe_mp` (numeric) există în bază, dar frontendul NU o citește:
--      `terenuri.js:335` calculează prețul din `pret_total / suprafata`.
--      Emailul face la fel — vezi comentariul din `lista_terenuri`.


-- ───────────────────────────────────────────────────────────────────────────
-- BLOC 1 — Funcția, versiunea 2
-- ───────────────────────────────────────────────────────────────────────────
-- ✋ NU RULA acest bloc înainte să-ți confirm că numele din BLOC 0 se
--    potrivesc. Dacă vreunul diferă, ți-l dau corectat.
--
-- ⚠️ Semnătura se schimbă (2 parametri → 3), deci vechea funcție se șterge
--    explicit. Apelurile cu 2 argumente merg mai departe neschimbate, fiindcă
--    al treilea are valoare implicită.
...
-- 2a. Funcția merge și lista arată cum trebuie?
--     Mă uit la `terenuri_lista` pe primul rând: trebuie să fie un tablou cu
--     cel mult 6 obiecte, cel mai nou primul, fiecare cu titlu și link-abil
--     prin `id`.
...
-- 2b. Cifrele VECHI au rămas identice după schimbare?
--     Dacă versiunea 2 a stricat ceva la numărătoare, se vede aici: numărul
--     de destinatari și suma terenurilor trebuie să fie exact ca la CSV 94
--     (62 de destinatari).
...
-- 2c. Un teren nu apare de două ori în lista aceluiași om?
select nume, email,
...
-- 2d. ⭐ DECIDE TEXTUL: câte terenuri au completat „câte apartamente"?
--     Dacă răspunsul e „aproape niciunul", rândul „3–4 apartamente" din
--     cardul de email nu se poate scrie ca element de bază — devine ceva ce
--     apare rar, deci textul trebuie construit fără el.
select
    count(*)                                                as terenuri_publice,
    count(*) filter (where nr_apartamente_min is not null)  as au_nr_apartamente,
    count(*) filter (where pret_total is not null
                       and suprafata is not null)           as au_pret_pe_mp,
    count(*) filter (where image_urls is not null
                        or image_url is not null)           as au_poza,
    count(*) filter (where analiza_generala_status is not null
                        or analiza_specifica_status is not null) as au_analiza
from public.terenuri
where deleted_at is null
  and status = 'approved';
-- Se citește ca proporție din `terenuri_publice`, nu în absolut.
