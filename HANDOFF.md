...
> Ultima actualizare: 5 iunie 2026 (sesiune v9: terenuri + utilizatori migrate la design v9 — reskin prin redefinirea variabilelor CSS, fără atins clase/JS/Supabase; accent ales: ink + teracotă #c2604a)
...
1. **Aliniere v9 — paginile rămase** (cel mai vizibil progres). Pagini nemigrate la design system v9: `grupuri`, `grup-nou`, `grup-details`, `grup-edit`, `analize`, `comanda-analiza`, `teren-details`. (DONE: `ce-este/*`, `povestea-noastra`, `parteneri`, `gdpr`, `termeni`, `politica-retur`, `contact`, `register`, `consultanta`, `devino-partener`, `ghid`, `news`, **`terenuri`**, **`utilizatori`**.) **La fiecare pagină verifică: placeholder-ul de header e exact `<div id="navigation">` (nu `nav`/`main-nav`) și există `<div id="footer">` — altfel chrome-ul nu se injectează (bug găsit pe ghid/news).**
...
- **Aliniere v9 — paginile rămase** (după `ce-este/`, `povestea-noastra`, `parteneri`, `gdpr`, `termeni`, `politica-retur`, `contact`, `register`, `consultanta`, `devino-partener`, `ghid`, `news`, `terenuri`, `utilizatori`, deja făcute): clusterul `grupuri.css` (grupuri + grup-nou + grup-details + grup-edit), analize, comanda-analiza, teren-details. (Rămase mai delicate — majoritatea cu fetch Supabase; comanda-analiza atinge zona de plăți.) Pe fiecare: link `css/apartamentual-v9.css`, scos `/nav.css` (mort), aliniat corpul la componentele v9. Atenție la paginile cu fetch Supabase (terenuri, grupuri, utilizatori, teren-details, comanda-analiza) — doar straturi vizuale, fără structură DOM; pe paginile unde markup-ul e generat din JS, NU schimba numele de clase (vezi pattern-ul de la `parteneri`).
