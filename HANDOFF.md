...
> Ultima actualizare: 5 iunie 2026 (sesiune v9: terenuri + utilizatori + paginile de detaliu teren-details/profile-view-new + comanda-analiza migrate la design v9; accent ales: ink + teracotă #c2604a)
...
1. **Aliniere v9 — paginile rămase** (cel mai vizibil progres). Pagini nemigrate la design system v9: clusterul `grupuri.css` (`grupuri`, `grup-nou`, `grup-details`, `grup-edit`) + clusterul „analize" (`analize`, `analiza-simplificata`, `analiza-detaliata`, `termeni-analize`). (DONE: `ce-este/*`, `povestea-noastra`, `parteneri`, `gdpr`, `termeni`, `politica-retur`, `contact`, `register`, `consultanta`, `devino-partener`, `ghid`, `news`, **`terenuri`**, **`utilizatori`**, **`teren-details`**, **`profile-view-new`**, **`comanda-analiza`**.) **La fiecare pagină verifică: placeholder-ul de header e exact `<div id="navigation">` (nu `nav`/`main-nav`) și există `<div id="footer">` — altfel chrome-ul nu se injectează (bug găsit pe ghid/news).**

   **⏭️ PROPUS pt. sesiunea următoare (proaspătă, după /clear): clusterul `grupuri.css`.** `grupuri.css` (838 linii) e folosit de **4 pagini**: `grupuri.html`, `grup-nou.html`, `grup-details.html`, `grup-edit.html`. Recolorarea variabilelor din `grupuri.css` le afectează pe toate 4 simultan — DAR `grup-details`/`grup-nou`/`grup-edit` au și **stiluri inline `<style>` proprii** (slate/coral hardcodat), deci dacă migrezi doar variabilele rămân pe jumătate (clase comune v9 + inline vechi). **De făcut împreună, într-o sesiune dedicată**: (1) reskin `grupuri.css` ca la terenuri/utilizatori, (2) în head-ul fiecăreia din cele 4: scos DM Sans + `/nav.css`, adăugat `css/apartamentual-v9.css` ÎNAINTE de `grupuri.css`, (3) rescris blocurile `<style>` inline din grup-details/grup-nou/grup-edit în paletă v9. `grup-details.html` are 4345 linii — context mare, posibil sesiune doar pt. el.
...
- **Aliniere v9 — paginile rămase** (după `ce-este/`, `povestea-noastra`, `parteneri`, `gdpr`, `termeni`, `politica-retur`, `contact`, `register`, `consultanta`, `devino-partener`, `ghid`, `news`, `terenuri`, `utilizatori`, deja făcute): clusterul `grupuri.css` (grupuri + grup-nou + grup-details + grup-edit) + clusterul „analize" (analize, analiza-simplificata, analiza-detaliata, termeni-analize). (Rămase mai delicate — unele cu fetch Supabase.) Pe fiecare: link `css/apartamentual-v9.css`, scos `/nav.css` (mort), aliniat corpul la componentele v9. Atenție la paginile cu fetch Supabase (terenuri, grupuri, utilizatori, teren-details, comanda-analiza) — doar straturi vizuale, fără structură DOM; pe paginile unde markup-ul e generat din JS, NU schimba numele de clase (vezi pattern-ul de la `parteneri`).
