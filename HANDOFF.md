...
> Ultima actualizare: 8 iunie 2026 (sesiune v9: clusterul grupuri — grupuri/grup-nou/grup-edit/grup-details migrate la design v9, commit 0c8a727; accent: ink + teracotă #c2604a)
...
1. **Aliniere v9 — paginile rămase** (cel mai vizibil progres). **A RĂMAS UN SINGUR CLUSTER:** „analize" (`analize`, `analiza-simplificata`, `analiza-detaliata`, `termeni-analize`) — toate DM Sans + nav.css + `<style>` propriu. (DONE: `ce-este/*`, `povestea-noastra`, `parteneri`, `gdpr`, `termeni`, `politica-retur`, `contact`, `register`, `consultanta`, `devino-partener`, `ghid`, `news`, `terenuri`, `utilizatori`, `teren-details`, `profile-view-new`, `comanda-analiza`, **`grupuri` + `grup-nou` + `grup-edit` + `grup-details`** (commit 0c8a727).) **La fiecare pagină verifică: placeholder-ul de header e exact `<div id="navigation">` (nu `nav`/`main-nav`) și există `<div id="footer">` dacă pagina încarcă footer.js — altfel chrome-ul nu se injectează (bug găsit pe ghid/news).**

   **⏭️ PROPUS pt. sesiunea următoare: clusterul „analize".** Pattern probabil = reskin în-pagină (au `<style>` propriu, ca devino-partener/consultanta) SAU `.v9-page` dacă sunt Tailwind. ⚠️ `analiza-simplificata`/`analiza-detaliata`/`comanda-analiza` ating zona „analize/plăți" — doar straturi vizuale, NU atinge JS/preț/flux. Notă: există un TODO vechi separat „Patch analiza-simplificata.html (înlocuire pop-up vechi cu link la /comanda-analiza.html)" — decizie de conținut, NU o amesteca cu reskinul v9.
